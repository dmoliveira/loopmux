...
- TUI controls: `h` hold/resume (non-consuming, alias `p`/`r`), `l` open active-list filter popup, `f` open fleet manager view, `R` renew counter, `n` next, `s`/`Ctrl+C` stop run, `q` quit run view.
